...
def data_analysis_2():
    ...
    print("\n***DataFrame composed of three componenets: index, columns, and data. Data also known as values.***")
    index = df.index
    columns = df.columns
    values = df.values
    ...
    print("\n15. Select all rows, and all columns, starting at column 2 (includes index column). Note: only first and last 30 records displayed:")
    a = df.iloc[:,:]
    print(a)
    ...
    print("\n29. Print element of (NumPy arrray) ndarray b in *second* row, *third* column:")
    print(b[1, 2])
    ...
    print("\n32. Final all passengers with name 'Allison' (using regular expressions):")
    for name in names:
        match = (re.search(r'(Allison)', name))
        if match:
            print(name)
    ...
    print("\n***Graph: Display age of the first 20 passengers (use code from previous assignment):***")
